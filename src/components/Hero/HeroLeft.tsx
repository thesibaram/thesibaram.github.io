  ...
  return (
    ...
    >
      ...
      <div className="hidden md:flex items-center gap-3 mb-8 w-full" style={{ maxWidth: 400 }}>
        <span className="font-mono text-[11px] text-[var(--px-muted)] tracking-[0.3em] uppercase">
          01 — Portfolio
        </span>
      ...
      {/* Photo frame */}
      <div className="relative w-full" style={{ maxWidth: "min(360px, 100%)" }}>
        ...
        <div className="overflow-hidden w-full" style={{ aspectRatio: "3/4", maxHeight: "clamp(240px, 55vw, 480px)" }}>
          {!imgError ? (
            <img
              src={about.image}
              alt="Sibaram Behera"
        ...
        <div className="absolute bottom-0 left-0 right-0" style={{ height: "3px", background: "var(--px-accent)" }} />
      </div>

      {/* Caption — desktop only */}
      <div className="hidden md:flex mt-6 items-start justify-between w-full" style={{ maxWidth: 400 }}>
        <div>
          <p className="font-mono text-[12px] font-semibold text-[var(--px-text)] tracking-wider">
            SIBARAM BEHERA
          </p>
          <p className="font-mono text-[11px] text-[var(--px-muted)] mt-1 tracking-wider">
            B.TECH EE · PMEC · BERHAMPUR, ODISHA
          </p>
        </div>
        <div className="flex items-center gap-1.5 px-2 py-1" style={{ border: "1px solid var(--px-accent)" }}>
          <span className="w-1.5 h-1.5 block flex-shrink-0" style={{ background: "#22C55E", animation: "blink 1s step-end infinite" }} />
          <span className="font-mono text-[11px] text-[var(--px-accent)] uppercase tracking-widest">Available</span>
        </div>
      </div>

      {/* Mobile-only caption */}
      <div className="flex md:hidden items-center justify-between w-full mt-3 px-1" style={{ maxWidth: "min(360px, 100%)" }}>
        <p className="font-mono text-[12px] text-[var(--px-muted)] tracking-wider">SIBARAM BEHERA · PMEC</p>
        <div className="flex items-center gap-1.5 px-2 py-0.5" style={{ border: "1px solid var(--px-accent)" }}>
          <span className="w-1.5 h-1.5 block flex-shrink-0" style={{ background: "#22C55E" }} />
          <span className="font-mono text-[11px] text-[var(--px-accent)] uppercase tracking-widest">Open</span>
        </div>
      </div>
    </motion.div>
  )
