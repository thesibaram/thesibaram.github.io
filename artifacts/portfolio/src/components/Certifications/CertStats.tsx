...
function useCountUp(target: number, duration = 1000, trigger: boolean) {
...
export function CertStats() {
  ...
  const v1 = useCountUp(earned, 1000, triggered)
  const v2 = useCountUp(inProgress, 1000, triggered)
  const v3 = useCountUp(planned, 1000, triggered)
  const v4 = useCountUp(domains, 1000, triggered)

  const stats = [
    { value: v1, label: "Earned" },
    { value: v2, label: "In Progress" },
    { value: v3, label: "Planned" },
    { value: v4, label: "Domains" },
  ]
    ...
    <div ref={ref} className="flex mt-8 border border-[var(--px-border)] divide-x divide-[var(--px-border)]">
      {stats.map(s => (
        <div key={s.label} className="flex-1 flex flex-col items-center py-4 gap-1">
          <span className="font-mono text-2xl font-bold text-[var(--px-accent)]">{s.value}</span>
